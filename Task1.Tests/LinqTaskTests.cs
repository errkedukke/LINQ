    ...
    public class Tests
    {
        ...
        public static IEnumerable<(string city, int averageIncome, int averageIntensity)> Linq9(IEnumerable<Customer> customers)
        {
            return customers
                ...
                .Select(cityGroup => (
                    city: cityGroup.Key,
                    averageIncome: (int)Math.Round(cityGroup.Average(c => c.Orders.Sum(o => o.Total)), MidpointRounding.AwayFromZero),
                    averageIntensity: (int)Math.Round(cityGroup.Average(c => c.Orders.Count()), MidpointRounding.AwayFromZero)
                ));
        }
